...
class AsgardeoClient {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        new URLSearchParams({
          ...
          scope: "internal_user_mgt_list internal_user_mgt_view internal_user_mgt_create internal_user_mgt_update internal_user_mgt_delete internal_group_mgt_view",
        }),
        {
          headers: {
      ...
      logger.error("Failed to get Asgardeo access token:", error.response?.data || error.message);
  ...
  async scim2Request(method, endpoint, data = null, params = {}) {
    ...
    try {
      ...
      logger.error(`SCIM2 API error (${method} ${endpoint}):`, error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async getGroupMembersDetailed(groupName) {
    ...
    const userPromises = members.map(member => 
      this.getUser(member.value).catch(err => {
        logger.warn(`Failed to fetch user ${member.value}:`, err.message);
        return null;
      })
    );
